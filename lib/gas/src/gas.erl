...
%%% associated with duplicate service process inclusion i.e two application
%%% in a release use the registered library process fs_elwrap.erl. </p>
%%%
...
%% @end
%%--------------------------------------------------------------------
load_config(Source, AppGroup, AppInstance) -> ok.
...
convert_list_string_type ([ValueHead | ValueTail]) when list (ValueHead) ->
	...
	AccOut = lists:append (TempString, StringX)
    end,
